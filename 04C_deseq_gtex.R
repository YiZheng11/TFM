...
# Overlapping genes
# --------------------
deg_lists <- readRDS(paste0("./extra/human/", tissue, "/", mitocarta, "/", grouping, "/venn_deg/DEG_lists.rds"))
...
common_genes <- presence_df %>%
  filter(M_40_49_vs_M_20_39 & M_60_79_vs_M_20_39) %>%
  pull(Gene)

common_genes <- presence_df %>%
  filter(M_50_59_vs_F_50_59 & M_60_79_vs_F_60_79) %>%
  pull(Gene)

res1 <- read.csv(file.path(DESEQ_OUT_DIR, "deseq_all_M_40_49_vs_M_20_39.tsv"), header=TRUE, sep='\t')
res2 <- read.csv(file.path(DESEQ_OUT_DIR, "deseq_all_M_60_79_vs_M_20_39.tsv"), header=TRUE, sep='\t')
...
table_common <- table_common %>%
  left_join(res1 %>%
              dplyr::select(symbol,
                     log2FC_1 = log2FoldChange, padj_1 = padj),
            by = "symbol") %>%
  left_join(res2 %>%
              dplyr::select(symbol,
                     log2FC_2 = log2FoldChange, padj_2 = padj),
            by = "symbol") %>%
  mutate(
    Direction = case_when(
      log2FC_1 > 0 & log2FC_2 > 0 ~ "up in males (both)",   # adapta al coding de tus contrastes
      log2FC_1 < 0 & log2FC_2 < 0 ~ "up in females (both)",
